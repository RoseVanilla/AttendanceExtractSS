...
st.title("📷 Automated OCR Attendance Tracker")
st.write("Upload a Zoom screenshot to extract participant attendance using OCR and update Google Sheets.")

# --- File & Input Controls ---
uploaded_file = st.file_uploader("Select Zoom Screenshot", type=["png", "jpg", "jpeg"])
col_input = st.text_input("Enter target column letter (e.g., A, B, C):").upper()

# Optional field to override default OCR API key if needed
api_key_input = st.text_input("OCR Space API Key (Leave blank to use default key):", type="password")
API_KEY = api_key_input.strip() if api_key_input.strip() else "K83980812088957"
...
if st.button("Process Attendance from Image"):
