...
if st.button("Process Attendance from Images"):
    ...
    try:
        with st.spinner("Processing participants and updating attendance..."):
            ...
            matched_hosts = []
            for p in Prtc:
                matched_h = next((h for h in host if part[s::]for s in range(3).upper().startswith(h.upper()for h in host), None)
                if matched_h:
                    ap = matched_h
            ...
            for part in Prtc:
                is_host = any(part[s::]for s in range(3).upper().startswith(h.upper()) for h in host)
